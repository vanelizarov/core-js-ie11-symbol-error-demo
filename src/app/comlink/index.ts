...
export function expose(obj: any, ep: Endpoint = self as any) {
  ep.addEventListener('message', (async (ev: MessageEvent) => {
    ...
    let returnValue
    try {
      const parent = path.slice(0, -1).reduce((o, prop) => o[prop], obj)
      const rawValue = path.reduce((o, prop) => o[prop], obj)
      switch (type) {
...
function createProxy<T>(
  ...
  const _proxy: Function = new Proxy(function () { }, {
    get(_target, prop) {
      if (prop === 'then') {
        if (path.length === 0) {
          return { then: () => _proxy }
...
function toWireValue(value: any): [WireValue, Transferable[]] {
  const handlers = Array.from(transferHandlers)
